  ...
  Future<void> _createMerchantUser() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance
        ...
        'username': username,
        'email':
            fakeEmail, // 🟢 เติมบรรทัดนี้เพิ่มเข้าไป เพื่อให้มีอีเมลบันทึกใน Firestore ครบถ้วน
        'role': 'merchant',
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text("สร้างบัญชีร้านค้าสำเร็จ!"),
      ...
      if (context.mounted) {
        ...
        if (e.code == 'email-already-in-use')
          message = "Username นี้มีคนใช้แล้ว กรุณาเปลี่ยนใหม่";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("ระบบจัดการแอดมิน",
            ...
            child: Column(
              ...
              children: [
                const Icon(Icons.storefront,
                    size: 60, color: Color(0xFF1D5A5D)),
                const SizedBox(height: 16),
                const Text("เพิ่มบัญชีร้านค้าใหม่",
                    style: TextStyle(
                        fontSize: 22,
                ...
                // ฟิลด์กรอกข้อมูล
                _buildTextField(
                    "ชื่อร้านค้า", Icons.store, _storeNameController),
                const SizedBox(height: 16),
                _buildTextField("Username (ใช้ตอนล็อกอิน)", Icons.person,
                    _usernameController),
                const SizedBox(height: 16),
                _buildTextField("รหัสผ่าน", Icons.lock, _passwordController,
                    isPassword: true),
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(12),
            borderSide: const BorderSide(color: Color(0xFF1D5A5D))),
      ),
    );
  }
}
